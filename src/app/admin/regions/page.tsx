...
export default function RegionsPage() {
  ...
  const handleEdit = (region: Region) => {
    ...
    setForm({ name: region.name, slug: region.slug, description: region.description || '' });
  };
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-8">
        <section className="rounded-3xl border border-[#d4cbb8] bg-white p-6 shadow-xl">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-[0.35em] text-[#b5150e] font-semibold">Regions</p>
              <h1 className="mt-3 font-serif text-3xl font-black">Manage region tags and local desks</h1>
            </div>
        ...
        <section className="grid gap-8 xl:grid-cols-[0.9fr_1.1fr]">
          <form onSubmit={handleSubmit} className="rounded-3xl border border-[#d4cbb8] bg-white p-6 shadow-xl space-y-4">
            <h2 className="font-serif text-xl font-black">{editingId ? 'Edit region' : 'Add region'}</h2>
            ...
            <button type="submit" className="w-full rounded-full bg-[#b5150e] px-4 py-3 text-sm font-semibold uppercase tracking-[0.25em] text-white hover:bg-[#8a0f09]">{editingId ? 'Save changes' : 'Add region'}</button>
          </form>

          <section className="rounded-3xl border border-[#d4cbb8] bg-white p-6 shadow-xl">
            {loading ? <p className="text-sm text-gray-600">Loading regions…</p> : (
              <div className="space-y-4">
                {regions.map((region) => (
                  <article key={region._id} className="rounded-2xl border border-[#efe7db] bg-[#faf8f4] p-4">
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      <div>
                        <p className="text-[10px] uppercase tracking-[0.35em] text-[#b5150e] font-semibold">{region.slug}</p>
                        <h3 className="mt-2 font-serif text-xl font-black text-[#0d0d0d]">{region.name}</h3>
                        <p className="mt-1 text-sm text-gray-600">{region.description || 'Regional desk used in story publishing.'}</p>
                      </div>
                      <div className="flex gap-3">
                        <button type="button" onClick={() => handleEdit(region)} className="rounded-full border border-[#d4cbb8] bg-white px-4 py-2 text-xs uppercase tracking-[0.25em] text-gray-700 hover:border-[#b5150e] hover:text-[#b5150e]">Edit</button>
                        <button type="button" onClick={() => handleDelete(region._id)} className="rounded-full bg-[#0d0d0d] px-4 py-2 text-xs uppercase tracking-[0.25em] text-white hover:bg-[#b5150e]">Delete</button>
                      </div>
                    </div>
                  </article>
                ))}
